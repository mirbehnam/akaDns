...
## 🖥️ نصب و اجرای ابزار (جداگانه برای ویندوز و لینوکس)

این پروژه دو دایرکتوری جدا دارد:

- `windows/` مخصوص ویندوز
- `linux/` مخصوص لینوکس

### 🪟 راهنمای نصب و استفاده در ویندوز

1. ابتدا زیپ پروژه را دانلود و از حالت فشرده خارج کنید.
2. وارد پوشه `windows/` شوید.
3. روی فایل `run-dns-scripts.bat` راست‌کلیک کنید و گزینه **Run as administrator** را بزنید.
4. سپس یکی از گزینه‌های زیر را انتخاب کنید:
...
> نکته: برای اجرای صحیح، دسترسی Administrator ضروری است.

### 🐧 راهنمای نصب و استفاده در لینوکس

1. زیپ پروژه را دانلود و استخراج کنید.
2. وارد پوشه `linux/` شوید.
3. در صورت نیاز، مجوز اجرا به اسکریپت‌ها بدهید:

```bash
chmod +x *.sh
```

4. سپس ابزار را با دسترسی روت اجرا کنید:

```bash
sudo ./run-dns-scripts.sh
```

گزینه‌های منو مشابه نسخه ویندوز هستند و همان قابلیت‌ها را ارائه می‌دهند.
